...
const Gallery = memo(function Gallery() {
  ...
  const fetchGalleryImages = useCallback(async () => {
    try {
      ...
      // Fetch 7 images for better space utilization
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/gallery/public`, {
        params: {
          limit: 7, // Increased from 6 to 7 images
        },
      })
  ...
  // Show first 7 images for preview
  const previewImages = useMemo(() => galleryImages.slice(0, 7), [galleryImages])
  ...
  // Close modal and navigation handlers
  const handleCloseModal = useCallback(() => {
  ...
  const handlePrevImage = useCallback(() => {
    ...
    setSelectedImageIndex((prevIndex) => {
      ...
      return prevIndex === 0 ? previewImages.length - 1 : prevIndex - 1
    })
  ...
  const handleNextImage = useCallback(() => {
    ...
    setSelectedImageIndex((prevIndex) => {
      if (prevIndex === null) return 0
      return prevIndex === previewImages.length - 1 ? 0 : prevIndex + 1
    })
  }, [previewImages.length])
  ...
  const handleImageClick = useCallback(
    (image) => {
      const index = previewImages.findIndex((img) => img._id === image._id)
      if (index !== -1) {
  ...
  // Memoized Gallery Card Component - Optimized Layout for 7 images
  const GalleryCard = memo(function GalleryCard({ image, index }) {
    const imageUrl = image.imageUrl ? `${import.meta.env.VITE_API_URL}${image.imageUrl}` : "/placeholder.svg"
        ...
        <div className="relative w-full h-full min-h-[16rem]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent group-hover:from-black/80 group-hover:via-black/40 transition-all duration-300">
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-center text-white p-6 text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ...
              <p className="text-sm text-gray-200 text-white">{image.description}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const currentImage = selectedImageIndex !== null ? previewImages[selectedImageIndex] : null
    ...
    <div className="min-h-screen">
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
        <div className="max-w-7xl mx-auto">
          {/* Header */}
          <div className="text-center mb-16">
            <h1 className="text-5xl md:text-6xl font-bold text-gray-800 mb-6">Media Library</h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
              Discover our collection of stunning visuals and memorable moments
            </p>
            {galleryImages.length > 0 && (
              <p className="text-sm text-gray-500 mt-2">Showing {previewImages.length} featured images</p>
            )}
          </div>
      ...
      {/* Enhanced Image Modal with Sliding Functionality */}
