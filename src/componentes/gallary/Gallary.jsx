...
const Gallery = memo(function Gallery() {
  ...
  const fetchGalleryImages = useCallback(async () => {
    try {
      ...
      // Fetch all active images for preview
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/gallery/public`, {
        params: {
          limit: 6, // Get 6 images for preview like original
        },
      })
  ...
  const previewImages = useMemo(() => galleryImages.slice(0, 6), [galleryImages])
  ...
  // Memoized Gallery Card Component - Fixed Layout
    ...
    return (
      ...
      >
        <div
          className={`
            relative w-full
            ${index === 0 ? "h-80 md:h-full" : index === 1 ? "h-64 md:h-full" : index === 2 ? "h-64 lg:h-[26rem]" : "h-64"}
          `}
        >
          <img
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-16">
            <h1 className="text-5xl md:text-6xl font-bold text-gray-800 mb-6">Media Gallery</h1>
          ...
          {/* Featured Gallery Grid - Fixed Layout */}
      ...
      {selectedImage && (
        ...
        >
          ...
          >
            ...
            <img
              src={
                selectedImage.imageUrl ? `${import.meta.env.VITE_API_URL}${selectedImage.imageUrl}` : "/placeholder.svg"
              }
